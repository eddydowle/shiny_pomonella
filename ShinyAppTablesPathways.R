                   ...
                   fluidRow(column(3,
                            ...
                            column(4, offset=1,
                                   radioButtons('within', label = 'pathway_within',
                                                choices = c("all","wnt","insulin","tor"),selected='wnt'),
                                   #                  fluidRow(
...
#      transcript<-strsplit(input$plot_var," +")[[1]][1]
      ...
      if ('LDxApple' %in% input$tableoptions | 'LDxHaw' %in% input$tableoptions){
        ...
        strsqlb<-cleanupldx(strsqlb)
        strsqlb<-c('SELECT annotation.snpId',strsqlb,paste("FROM annotation, ",input$population,"poolLDx, feature_alias WHERE (%s) AND feature_alias.loc = annotation.loc AND annotation.snpId=",input$population,"poolLDx.snpId;",sep=''))
        strsqlb<-paste(strsqlb,collapse=', ')
        strsqlb<-gsub(', FROM ',' FROM ',strsqlb)
 #       transcript<-strsplit(input$plot_var," +")[[1]][1]
        sqlb <- sprintf(strsqlb,avector.str)
...
#      transcript<-strsplit(input$plot_var," +")[[1]][1]
 ...
 #     transcript<-strsplit(input$plot_var2," +")[[1]][1]
    ...
    #    transcript<-strsplit(input$plot_var2," +")[[1]][1]
        sql2b <- sprintf(strsql2b,avector.str)
